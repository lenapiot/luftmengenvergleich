...
def _choose_schema_source() -> list[Path]:
    """
    Empfohlener Weg:
    Ordner auswählen -> alle PDFs im Ordner werden verwendet.

    Alternativ:
    Einzelne PDFs auswählen.
    """
    choice = simpledialog.askinteger(
        "Strangschemata auswählen",
        (
            "Wie möchtest du die Strangschemata auswählen?\n\n"
            "1 = Ordner auswählen (empfohlen)\n"
            "2 = Einzelne PDFs auswählen"
        ),
        minvalue=1,
        maxvalue=2,
    )
    ...
    if choice == 1:
        ...
        if not schema_paths:
            messagebox.showerror(
                "Keine PDFs gefunden",
                (
                    "Im ausgewählten Ordner wurden keine PDF-Dateien gefunden."
                ),
            )
            return []
...
def _choose_heating_pdfs() -> list[Path]:
    return _choose_pdf_folder(
        "Ordner mit Heizlast-Grundrissen auswählen",
    )


def _choose_cooling_pdfs() -> list[Path]:
    return _choose_pdf_folder(
        "Ordner mit Kühllast-Grundrissen auswählen",
    )
...
def main() -> None:
    ...
    schema_paths = _choose_schema_source()
